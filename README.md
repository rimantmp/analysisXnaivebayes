...
## Menjalankan Tanpa MySQL

Aplikasi analisis dapat dijalankan sementara tanpa penyimpanan database:
...
Mode ini hanya sesuai untuk pengembangan. Data pipeline tersimpan di memori dan
akan hilang ketika aplikasi berhenti.
...
Tutup dan buka kembali VS Code, kemudian periksa:
...
Pastikan commit pertama sudah berhasil:

```powershell
git add .
git commit -m "Initial commit analisis sentimen"
git branch -M main
...
- pengguna memiliki izin membuat database dan tabel;
- port `3306` tidak digunakan service lain.

### PowerShell tidak dapat mengaktifkan `.venv`
...
atau jalankan interpreter virtual environment secara langsung:

```powershell
.\.venv\Scripts\python.exe run.py
```

### Grafik atau tampilan tidak termuat

Tailwind CSS dan Chart.js dimuat dari CDN. Pastikan browser memiliki koneksi
internet dan CDN tidak diblokir.

## Keamanan

- Jangan commit file `.env`.
- Jangan gunakan password default pada server.
- Gunakan `SECRET_KEY` acak dan panjang.
- Gunakan pengguna MySQL khusus aplikasi dengan izin minimum.
- Jalankan aplikasi melalui HTTPS ketika dipublikasikan.
- Mode debug Flask tidak boleh digunakan pada production.
- Batasi akses database hanya dari host yang diperlukan.
...
```text
https://github.com/rimantmp/analysisXnaivebayes.git
```
...
git commit -m "Initial commit analisis sentimen"
git branch -M main
git remote add origin https://github.com/rimantmp/analysisXnaivebayes.git
git push -u origin main
```
...
```powershell
git remote set-url origin https://github.com/rimantmp/analysisXnaivebayes.git
git push -u origin main
```
...
- State pipeline bersifat global in-memory dan paling sesuai untuk penggunaan
  single-user atau pengembangan lokal.
- Metadata dan hasil analisis tersimpan di MySQL, tetapi objek model belum
  dipersistenkan sebagai file.
- Untuk deployment multi-user, state pipeline perlu dipisahkan berdasarkan
  pengguna atau dataset dan model sebaiknya disimpan menggunakan `joblib`.
- Untuk production, gunakan WSGI server seperti Waitress atau Gunicorn dan
  jangan memakai development server Flask.

## Lisensi

Belum ada lisensi khusus yang ditetapkan. Tambahkan file `LICENSE` sebelum
repository didistribusikan sebagai perangkat lunak open source.
